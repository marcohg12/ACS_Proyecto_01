...
describe("PublicationService", () => {
  ...
  it("should throw an exception when trying to get a publication with a non-existing ID", async () => {
    ...
    await expect(
      publicationAdmin.getPublication(nonExistingId)
    ).rejects.toThrow(`Publication with ID ${nonExistingId} not found`);
  });
  ...
  it("should get all publications with an existing category", async () => {
    ...
    const publications = await publicationAdmin.getPublicationsByCategory(
      categoryId
    );
    expect(publications).toBeTruthy();
    expect(publications.length).toBeGreaterThan(0);
  ...
  it("should throw an exception when trying to get publications with a non-existing category ID", async () => {
    ...
    await expect(
      publicationAdmin.getPublicationsByCategory(nonExistingCategoryId)
    ).rejects.toThrow(
      `No publications found for category ID ${nonExistingCategoryId}`
    );
  });
  ...
  it("should get all publications with an existing tag", async () => {
    ...
    expect(publications.length).toBeGreaterThan(0);
    publications.forEach((publication) => {
      expect(publication.tags.some((tag: string) => tags.includes(tag))).toBe(
        true
      );
    });
  });
  ...
  it("should throw an exception when trying to get publications with a non-existing tag", async () => {
    ...
    await expect(
      publicationAdmin.getPublicationsByTags([nonExistingTag])
    ).rejects.toThrow(`No publications found for tags ${nonExistingTag}`);
  });
  ...
  it("should register a publication if receiving a publication object", async () => {
    ...
    await expect(
      publicationAdmin.registerPublication(publication)
  ...
  it("should reject to register a publication if receiving an object different from publication", async () => {
    ...
    await expect(
      publicationAdmin.registerPublication(invalidObject)
    ).rejects.toThrow("Invalid publication object");
  });
  ...
  it("should update a publication if receiving a publication object", async () => {
    ...
    await expect(
      publicationAdmin.updatePublication(publication)
    ).resolves.not.toThrow();
  });
  ...
  it("should reject to update a publication if receiving an object different from publication", async () => {
    ...
    await expect(
      publicationAdmin.updatePublication(invalidObject)
    ).rejects.toThrow("Invalid publication object");
  });
  ...
  it("should delete a publication with an existence ID", async () => {
    ...
    await expect(
      publicationAdmin.deletePublication(publicationId)
    ).resolves.not.toThrow();
  });
  ...
  it("should reject to delete a publication with a non-existent ID", async () => {
    ...
    await expect(
      publicationAdmin.deletePublication(nonExistentPublicationId)
